...
def render_empty_history():
    ...
    with col1:
        if st.button("📁 Upload Document", key="upload_from_history"):
            st.switch_page("app.py")  # Navigate to main dashboard
    with col2:
        if st.button("🏠 Go to Dashboard", key="dashboard_from_history"):
            st.switch_page("app.py")  # Navigate to main dashboard
...
def render_document_grid():
    ...
    total_docs = len(st.session_state.document_history) if hasattr(st.session_state, 'document_history') else 0
    st.markdown(f"""
    <div style="margin: 2rem 0 1rem 0; color: #6C757D;">
        Showing {len(filtered_docs)} of {total_docs} documents
    </div>
    """, unsafe_allow_html=True)
...
def render_document_card(doc, index):
    ...
    with col2:
        if st.button("📖 Analyze", key=f"analyze_{card_key}"):
            ...
            st.session_state.highlighted_text = doc['text']  # Set highlighted text for analysis
