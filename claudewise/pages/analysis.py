...
from utils import (
    ...
    highlight_entities_in_text, test_tts_connection
)
...
def render_document_preview():
    ...
    if not hasattr(st.session_state, 'highlighted_text') or not st.session_state.highlighted_text:
...
def render_voice_panel():
    ...
    if not hasattr(st.session_state, 'document_summary') or not st.session_state.document_summary:
    ...
    with st.expander("🔧 TTS Debugging & Test", expanded=False):
...
def render_analysis_section():
    ...
    with col1:
        ...
        if not hasattr(st.session_state, 'document_analysis') or not st.session_state.document_analysis:
...
def render_summary_section():
    ...
    if not hasattr(st.session_state, 'detailed_summary') or not st.session_state.detailed_summary:
    ...
    with col2:
        if hasattr(st.session_state, 'detailed_audio_data') and st.session_state.detailed_audio_data:
            st.markdown(f"""
